...
int main() {
    ...
    assert(state.stop_reason == psprecomp::StopReason::running);
    assert(psprecomp::load32(state, 0x1004) == 0x89abcdefU);
    assert(memory[4] == 0xef && memory[7] == 0x89);

    (void)psprecomp::load16(state, 0x1001);
    assert(state.stop_reason == psprecomp::StopReason::memory_fault);
    assert(state.fault_address == 0x1001);
    assert(psprecomp::arithmetic_shift_right(0x80000000U, 1) == 0xc0000000U);
    assert(psprecomp::arithmetic_shift_right(0x7fffffffU, 4) == 0x07ffffffU);
    assert(psprecomp::rotate_right(0x12345678U, 8) == 0x78123456U);
    psprecomp::set_f32(state, 3, 1.5F);
    assert(psprecomp::f32(state, 3) == 1.5F);
    assert(psprecomp::rounded_word(1.6F, 0) == 2U);
    assert(psprecomp::rounded_word(-1.6F, 1) == 0xffffffffU);
    psprecomp::compare_f32(state, 0, 1.0F, 2.0F, 0x0c);
    assert(psprecomp::fpu_condition(state, 0));
    ...
    assert(memory[2] == 0x11 && memory[3] == 0x22);
    assert(memory[4] == 0x33 && memory[5] == 0x44);
    ...
    assert(psprecomp::vfpu_float(state, psprecomp::vfpu_index(0, 1)) == 2.0F);
    ...
    for (int column = 0; column < 4; ++column) {
        for (int row = 0; row < 4; ++row) {
            assert(transposed[column * 4 + row] ==
    ...
    assert(psprecomp::apply_psp_relocations(
               relocated.data(), relocated.size(), 0x08800000U, segments, 1,
               relocations, 4) == psprecomp::RelocationResult::success);
    assert(psprecomp::detail::read_u32(relocated.data(), 0) == 0x08800020U);
    assert(psprecomp::detail::read_u32(relocated.data(), 4) == 0x0a200010U);
    assert(psprecomp::detail::read_u32(relocated.data(), 8) == 0x3c080881U);
    assert(psprecomp::detail::read_u32(relocated.data(), 12) == 0x25088000U);
}
